...
## Notes
- **2026-06-26 standup (work re-org):** **Email/outreach** confirmed as Cooper's single-owner workstream (analyze real emails → templates → champion-finder flow); **website/logo** is his side project. Noted he used to just wait around to catch Evan in person — Evan's guidance: email him (inbox = to-do list), especially for weekend blockers; chat won't get a reliable response. Cooper voiced appreciation for Evan's coordinator/support role (sets direction + deadlines without micromanaging).
...
- **2026-06-19 1:1 — Buyer hub design review (Gemini export; Gemini attributed all speech to "Cooper Tenney" — transcript is multi-speaker; reviewer's directives are mislabeled as Cooper; attribution kept thin):** Demoed the buyer hub interface and took design feedback. Key decisions:
  - **Account list:** Single scrollable list sorted by days to renewal; engagement level (not engaged / somewhat engaged / very engaged) shown per account row.
  - **Drill-down behavior:** Once clicking into an account, hide the aggregate summary panel so the user focuses on account-level detail.
  - **Documents section:** Expand the "Contracts" section to a broader "Documents" section covering MSAs, product briefs, and previous MSA versions.
  - **Change tracking:** Use "highlights" (color-coded annotations on the document) instead of redlines for tracking changes between contract versions.
  - **Meeting scheduler:** Add a BookIt/Calendly-style meeting scheduler within the buyer interface so buyers can self-schedule directly.
  - **Admin view:** Stakeholder engagement tracking + doc-open status per document flagged as important for the admin/seller view.
  - **Engagement heat map:** Crazy Egg-style section-level heat map for buyer engagement visualization discussed as a potential feature.
  - **Privacy concern:** Declined Fireflies bot request for domain-level admin access to all meetings — confidentiality concern. This directly motivated setting up Gemini as a backup transcription source.

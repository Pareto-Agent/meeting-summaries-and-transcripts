...
> **Day-to-day ownership reorganized 2026-06-26** into autonomous single-owner *workstreams* (see Decision Log): **Prototype → Parth**, **Qualification → Aaron**, **Email/outreach → Cooper**, **Architecture + eval → Dean**. The module table above describes the product components; the workstreams are how the work is actually divided now to avoid collisions on the merged codebase.
...
- [ ] **Data pull (blocker):** Contract PDFs cannot be downloaded through the LeanData MCP — the MCP points to them but they can't be pulled. Need an alternate way to fetch full contract PDFs (required for buyer-engagement + qualification). Also: data pulling should serve ALL workstreams (contracts, emails, etc.), not only what qualification needs (2026-06-26 standup)
- [ ] **Dean:** Own product architecture + build the eval/QA system for AI outputs; Evan setting up time with technical advisors to scope the final-product architecture (2026-06-26 standup)
- [ ] **Parth:** Take over the front-end prototype workstream (UI iterations + add the missing negotiation & close models); email Evan the slide-deck link → Evan to give feedback by the weekend (2026-06-26 standup)
- [ ] **Whole team:** When blocked (especially weekends), **email Evan** (with the link) — his inbox is his to-do list; Google Chat/Slack messages will not reliably get a response (2026-06-26 standup)

## Decision Log
### 2026-06-26 — Standup (work re-org into autonomous single-owner workstreams)
- **Decision:** Break the merged product back into separate, single-owner workstreams to restore autonomy and stop four people from colliding on one codebase. Ownership:
  - **Prototype** (front-end demo for AJ/others; iterate UI + add the missing negotiation & close models) → **Parth**
  - **Qualification** (LeanData data → qualify/categorize; pricing/modeling later) → **Aaron**
  - **Email / outreach** (analyze real emails → templates → champion-finder flow on real data) → **Cooper**
  - **Product architecture + eval systems** (final-product architecture + AI output validation/QA "evals") → **Dean**
  - Side projects: **sales pitch deck** iteration → **Parth**; **website/logo** → **Cooper**; engineering-sprints/scrum board → deprioritized; Luncher → deprioritized. Target: ~1 major + 1 minor per person.
- **Rationale:** The team is in the "storming" stage (Tuckman forming→storming→norming→performing); with four people on one merged product the collision/blocking cost is high. Evan cited a peer company that found adding *any* second person to one product (senior/junior/intern) slowed it down — "one person owns the entire product." The original one-agent-per-person design (no data passing) worked well for ~a week until the merge forced everyone onto one product. Prototype and real-data workstreams should NOT be merged short-term — merging recreates the conflict. Eval systems are called the single most important thing (this week's literature + the LeanData contact, who believes a strong eval system removes the need to hand-define stages/steps — Parth to test).
...
## Notes
- **Team dynamics framing (Evan, 2026-06-26 standup):** Used the Tuckman model (forming → storming → norming → performing → adjourning) to explain current friction — the team is in "storming" (moving through stages in weeks, not the usual months/quarters). Fix = restore autonomy via single-owner workstreams (see Decision Log). High-performance ingredients Evan named: autonomy, mutual respect/trust, camaraderie, and visible tangible results.
- **Comms norm (Evan, 2026-06-26):** Evan is **email-centric**, not chat — his inbox doubles as his to-do list. To unblock him (especially over weekends), email him the ask/link; Google Chat/Slack won't reliably get a response. Reason given: external constituents (board, investors, customers) live in email, and the brain can only keep one primary channel.
- **CEO role thesis (Evan, 2026-06-26):** the CEO's job = identify the single most critical issue (funding / sales / hiring / keeping the lights on) and prioritize+delegate resources to it; stay "floatable" (not tied to ongoing work) so you can absorb crises. Evan's two current priorities: (1) keep the team productive & aligned, (2) hire a CTO. He's settled into a coordinator role (decided not to own a build module himself once the team's pace outran that).
- **Prototype is a UI demo, not the product (reaffirmed 2026-06-26):** the front-end prototype "will never be a functional thing" but is valuable for demoing to AJ and others; real-data work is built separately and must not be merged into the prototype short-term.
- **External validation (Evan, 2026-06-26):** showed the prototype to Hannah, who was "almost crying" at how good it is versus a prior build ("Maritzo"); Juliet (a notably tough critic) was impressed.
